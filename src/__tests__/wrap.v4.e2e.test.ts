...
const fixtureFolderName = path.join("fixture", "4.0");
const validFileName = "valid-tradetrust-document.json";
const validFileNameWithCustomSchema = "valid-custom-schema-document.json";
const invalidFileName = "invalid-tradetrust-document.json";
...
describe("wrap", () => {
  ...
  describe("wrap with directory input", () => {
    describe("without schema", () => {
      it("should issue document when folder contain one valid tradetrust document", async () => {
        const inputDirectory = tmp.dirSync();
        const outputDirectory = tmp.dirSync();
        fs.copyFileSync(
          path.resolve(__dirname, fixtureFolderName, validFileName),
          path.resolve(inputDirectory.name, "valid-tradetrust-document.json")
        );
        ...
        const file = JSON.parse(
          fs.readFileSync(path.resolve(outputDirectory.name, "valid-tradetrust-document.json"), {
            encoding: "utf8",
          })
        );
      ...
      it("should wrap documents when folder contain multiple valid tradetrust documents", async () => {
        const inputDirectory = tmp.dirSync();
        const outputDirectory = tmp.dirSync();
        fs.copyFileSync(
          path.resolve(__dirname, fixtureFolderName, validFileName),
          path.resolve(inputDirectory.name, "valid-tradetrust-document-1.json")
        );
        fs.copyFileSync(
          path.resolve(__dirname, fixtureFolderName, validFileName),
          path.resolve(inputDirectory.name, "valid-tradetrust-document-2.json")
        );
        fs.copyFileSync(
          path.resolve(__dirname, fixtureFolderName, validFileName),
          path.resolve(inputDirectory.name, "valid-tradetrust-document-3.json")
        );
        ...
        const file1 = JSON.parse(
          fs.readFileSync(path.resolve(outputDirectory.name, "valid-tradetrust-document-1.json"), {
            encoding: "utf8",
          })
        );
        const file2 = JSON.parse(
          fs.readFileSync(path.resolve(outputDirectory.name, "valid-tradetrust-document-2.json"), {
            encoding: "utf8",
          })
        );
        const file3 = JSON.parse(
          fs.readFileSync(path.resolve(outputDirectory.name, "valid-tradetrust-document-3.json"), {
            encoding: "utf8",
          })
        );
        expect(merkleRoot).toHaveLength(64);
      ...
      it("should wrap documents individually when folder contain multiple valid tradetrust documents and batch is false", async () => {
        const inputDirectory = tmp.dirSync();
        const outputDirectory = tmp.dirSync();
        fs.copyFileSync(
          path.resolve(__dirname, fixtureFolderName, validFileName),
      ...
      it("should not issue document when folder contain one invalid tradetrust document", async () => {
        const inputDirectory = tmp.dirSync();
        const outputDirectory = tmp.dirSync();
        fs.copyFileSync(
          path.resolve(__dirname, fixtureFolderName, invalidFileName),
          path.resolve(inputDirectory.name, "invalid-tradetrust-document.json")
        );
        await handler({
          rawDocumentsPath: inputDirectory.name,
          outputDir: outputDirectory.name,
        ...
        const filepath = path.resolve(inputDirectory.name, "invalid-tradetrust-document.json");
        expect(signaleErrorSpy).toHaveBeenCalledWith(
          `Document ${filepath} is not valid against open-attestation schema`
        );
        expect(fs.readdirSync(outputDirectory.name)).toHaveLength(0);
      });
      it("should not issue documents when folder contain multiple invalid tradetrust document", async () => {
        const inputDirectory = tmp.dirSync();
        const outputDirectory = tmp.dirSync();
        fs.copyFileSync(
          path.resolve(__dirname, fixtureFolderName, invalidFileName),
          path.resolve(__dirname, inputDirectory.name, "invalid-tradetrust-document-1.json")
        );
        fs.copyFileSync(
          path.resolve(__dirname, fixtureFolderName, invalidFileName),
          path.resolve(__dirname, inputDirectory.name, "invalid-tradetrust-document-2.json")
        );
        fs.copyFileSync(
          path.resolve(__dirname, fixtureFolderName, invalidFileName),
          path.resolve(__dirname, inputDirectory.name, "invalid-tradetrust-document-3.json")
        );
        ...
        const filepath = path.resolve(inputDirectory.name, "invalid-tradetrust-document-1.json");
        expect(signaleErrorSpy).toHaveBeenCalledWith(
          `Document ${filepath} is not valid against open-attestation schema`
        );
        expect(fs.readdirSync(outputDirectory.name)).toHaveLength(0);
    ...
    describe("with schema", () => {
      it("should not issue documents when folder contain one valid tradetrust that is not valid against the local schema provided", async () => {
        const inputDirectory = tmp.dirSync();
        const outputDirectory = tmp.dirSync();
        fs.copyFileSync(
      ...
      it("should issue documents when folder contain one valid tradetrust that is also valid against the remote schema provided", async () => {
        const inputDirectory = tmp.dirSync();
        const outputDirectory = tmp.dirSync();
        fs.copyFileSync(
      ...
      it("should not issue documents when folder contain one valid tradetrust that is not valid against the remote schema provided", async () => {
        const inputDirectory = tmp.dirSync();
        const outputDirectory = tmp.dirSync();
        fs.copyFileSync(
          path.resolve(__dirname, fixtureFolderName, invalidCustomFileName),
  ...
  describe("wrap with file input", () => {
    describe("without schema", () => {
      it("should issue document when given valid tradetrust document", async () => {
        const outputDirectory = tmp.dirSync();
        const merkleRoot = await handler({
      ...
      it("should not issue document when given invalid tradetrust document", async () => {
        const outputDirectory = tmp.dirSync();
    ...
    describe("with schema", () => {
      ...
      it("should not issue document when given valid tradetrust document that is not valid against the local schema provided", async () => {
        const outputDirectory = tmp.dirSync();
        await handler({
      ...
      it("should issue document when given valid tradetrust document that is also valid against the remote schema provided", async () => {
        const outputDirectory = tmp.dirSync();
        const merkleRoot = await handler({
          rawDocumentsPath: path.resolve(__dirname, fixtureFolderName, validFileNameWithCustomSchema),
          outputDir: outputDirectory.name,
      ...
      it("should not issue document when given tradetrust document that is not valid against the remote schema provided", async () => {
        const outputDirectory = tmp.dirSync();
        await handler({
          rawDocumentsPath: path.resolve(__dirname, fixtureFolderName, invalidCustomFileName),
          outputDir: outputDirectory.name,
